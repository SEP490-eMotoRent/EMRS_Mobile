...
export class BranchRemoteDataSourceImpl implements BranchRemoteDataSource {
  ...
  async getByVehicleModelId(vehicleModelId: string): Promise<BranchModelDetailResponse[]> {
    const response = await this.axiosClient.get<ApiResponse<BranchModelDetailResponse[]>>(
