...
export const SwapOldVehicleScreen: React.FC = () => {
  ...
  const {
    ...
    newVehicleOdometerKm,
    newVehicleBatteryPercentage,
  } = route.params;
  const [odometer, setOdometer] = useState(
    startOldVehicleOdometerKm?.toString() || ""
  );
  ...
  const validate = (): boolean => {
    ...
    if (!odometer || odometer.trim() === "") {
      ...
      if (isNaN(odometerNum) || odometerNum < 0) {
        ...
        startOldVehicleOdometerKm !== undefined &&
        odometerNum < startOldVehicleOdometerKm
      ) {
  ...
  const handleNext = () => {
    ...
    captureChecklist().then((checklistUri) => {
      ...
      navigation.navigate("SwapNewVehicle", {
        newVehicleOdometerKm,
        newVehicleBatteryPercentage,
      });
    });
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView ref={scrollViewRef} contentContainerStyle={styles.content}>
        <ScreenHeader
          title="Xe cũ"
          subtitle="Bước 2/4 · Nhập thông tin xe cũ"
